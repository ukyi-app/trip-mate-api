...
1. **전체 스위트 green** — `bun run test`:
   ...
   ```
   baseline(main) 590 passed → **+26 신규**(coverage·currencies·expense DTO·trip net·me/invites·migrate-only 28행), 회귀 0.
...
- I-1: coverage(SUPPORTED⊇seed 정적), provider.test 28 정합, schema-introspection 28, **migrate-only(seed 없이)→28행**(부팅 경로, R-1 락).
...
- **release gate**: r1 needs-attention 1(R-1: 마이그레이션-only DB가 원본 9종 놓침) → 0008 전 28종 +
  migrate-only 회귀 테스트 반영 → r2(다음).
